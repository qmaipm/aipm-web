...
// 视觉完全沿用 _agents-app/globals.css 的 VI:section-header 体系 + product-card 卡片语言 + cta-section 渐变带
...
const PRODUCTION = [
  { t: "准确", d: "指标与口径使用统一定义,结果可以回到来源复核。" },
  { t: "一致", d: "同一工作流与口径下,结果与报告结构跨项目保持一致。" },
  { t: "可追溯", d: "数据读取、流程运行、系统调用与人工确认全程留有记录。" },
];

const SCENARIOS = [
  { name: "供应商自动对账", href: "/scenarios/reconciliation", d: "核对合同、工作量与账单,标记异常并准备确认材料。" },
  { name: "智能派单", href: "/scenarios/dispatch", d: "结合位置、等级与负载形成派单建议并跟踪执行。" },
  { name: "品质巡检", href: "/scenarios/inspection", d: "依据统一标准检查现场,关联问题、证据与整改任务。" },
  { name: "管理层问询", href: "/scenarios/exec-query", d: "基于统一口径回答运营问题,提供可复核的数据来源。" },
  { name: "费用审批", href: "/scenarios/utility-bill", d: "汇总能耗、合同与历史数据,识别异常并辅助审批。" },
  { name: "报修客服", href: "/scenarios/repair-bot", d: "识别报修、创建工单、通知相关人员并跟踪处理结果。" },
];
...
export default function MatrixSections() {
  return (
    <>
        ...
        <div className="section-container">
          <div className="section-header">
            ...
            <p className="section-desc">
              ...
              <Link href="/products/fmclaw" style={{ color: "var(--vi-blue)", fontWeight: 600 }}>FMClaw 平台</Link>之上。
            </p>
          </div>
          <div className="mx-grid-4">
            {PLATFORM.map((p) => (
              <Link className="mx-card" href={p.href} key={p.href}>
                <span className="mx-rel">{p.rel}</span>
                <h3>{p.name}</h3>
                <p>{p.d}</p>
                <span className="mx-go">了解更多 →</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="mx-section mx-gray" id="production">
        <div className="section-container">
          <div className="section-header">
            <span className="section-eyebrow">BUILT FOR OPERATIONS</span>
            <h2 className="section-title">进入业务系统之后,结果必须可以复核</h2>
          </div>
          <div className="mx-grid-3">
            {PRODUCTION.map((c) => (
              <div className="mx-card" key={c.t}>
                <h3>{c.t}</h3>
                <p>{c.d}</p>
              </div>
            ))}
      ...
      <section className="mx-section mx-white" id="start-scenarios">
        <div className="section-container">
          <div className="section-header">
          ...
          <div className="mx-grid-3">
            {SCENARIOS.map((s) => (
              <Link className="mx-card" href={s.href} key={s.href}>
                <h3>{s.name}</h3>
                <p>{s.d}</p>
                <span className="mx-go">查看场景 →</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {/* FAQ */}
      <section className="mx-section mx-gray" id="faq">
        <div className="section-container">
          <div className="section-header">
      ...
      {/* GET STARTED — 复用原 cta-section 渐变带 */}
