...
export const metadata: Metadata = pageMetadata("/ai-service/security", {
  ...
  description:
    "AI 安保服务不讲概念,只做三件具体的事:每天上千条视频预警先经 AI 预审,让每一条都真正被看过——预警图片经中间数据池接入 FMClaw 数据本体,识别后直接开工单派人,不需要改造摄像头、不需要开接口;消防巡检按人单合一执行,IoT 在场核验,守住安全红线;巡检狗承担园区巡逻,顺带消防、卫生与车辆违停检查。配合无人值守停车与门禁联动,安保成本结构同步优化。由启盟科技旗下自营物业公司爱物管交付。",
  keywords: [
    "AI 安保服务",
    "安保外包",
    "保安服务公司",
...
const compare: { k: string; old: string; now: React.ReactNode }[] = [
  {
    ...
    now: <><b>AI 先看一遍</b>——海康、大华等系统的预警图片经中间数据池接入,AI 预审后滤掉误报,真正需要人处置的直接派单到岗</>,
  },
  {
    k: "消防巡检",
...
const faq = [
  {
    ...
    a: "区别不在人,在三件具体的事:一是视频预警处置——监控系统(海康、大华等)的预警图片经中间数据池接入 FMClaw 数据本体,AI 预审把每天上千条里的误报滤掉,真正的异常直接开工单派到岗位,每一条预警都真正被看过;二是消防巡检按「人单合一」执行,IoT 在场核验、漏检自动预警,守住安全红线;三是巡检狗承担园区例行巡逻,顺带做消防、卫生与车辆违停检查。这项服务由启盟科技旗下自营物业公司爱物管交付。",
  },
  {
    q: "监控预警太多看不过来,有什么解决办法?",
    a: "这是几乎所有项目的共同困境:监控系统一天报上千条预警,靠人一条条看完,从人性上就做不到。解决办法是让 AI 先看一遍:预警图片经中间数据池接入 FMClaw,AI 逐张预审、滤掉误报,真正需要处置的少数直接开工单派到安保岗位。不需要更换摄像头、不需要改造监控系统,也不接视频流,原有系统一动不动。",
  },
  {
    q: "消防设施巡检怎么保证真的巡了?",
  ...
  {
    q: "需要更换现有的监控和门禁系统吗?",
    a: "不需要。AI 预警预审只用经中间数据池接入的预警图片,不接视频流,海康、大华等系统都不需要改造、不需要开接口,图片范围由你在池子这一层自己限定;门禁、道闸联动走标准接口,是否打通由你决定。我们的原则是尽量利用你已有的设备投资,把钱花在计算与调度上,而不是重复采购硬件。",
  },
];
...
const SERVICE_LD = {
  ...
  description:
    "监控预警图片经中间数据池接入 FMClaw 数据本体做 AI 预审、识别后直接开工单派单(不改造原有监控系统)、消防巡检人单合一、巡检狗例行巡逻,配合无人值守停车与门禁联动的安保服务。",
  areaServed: "CN",
      ...
      <section className="av-hero">
        ...
        <div className="wrap av-hero-top">
          ...
          <div className="av-proof reveal">
            <span><b>不接视频流</b>，预警图片经数据池接入</span>
            <span className="sep" />
          ...
          <div className="sec-funnel reveal">
            <div className="sec-funnel__row">
              ...
              <div className="sec-funnel__stage mid">
                ...
                <span className="lb">预警图片经中间数据池进来，FMClaw 逐张先审一遍，滤掉误报</span>
              </div>
              <div className="sec-funnel__arrow" aria-hidden="true">
                <svg width="22" height="22" viewBox="0 0 16 16"><path d="M3 8h10M9 4l4 4-4 4" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </div>
              <div className="sec-funnel__stage out">
                <span className="num">直接派单</span>
                <span className="lb">调用工具箱开工单、派到对应岗位——不是多一条预警，是多一张已经派下去的活</span>
              </div>
            </div>
            <p className="sec-funnel__foot">
              不需要更换摄像头，也不接视频流。<b>预警图片经中间数据池接入 FMClaw 数据本体</b>——原有系统一动不动，图片范围由你自己限定。
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="wrap">
          ...
          <div className="avc-cases">
            ...
            <Link className="avc-case reveal" href="/cases/campus-cctv-photo-ai-review">
              <span className="tag">智慧园区 · 识别完直接派单</span>
              <h3>监控预警没人看得完，识别完直接派单</h3>
              <p>预警图片经数据池进数据本体，识别后由工具箱开工单。一次电动车充电冒烟，3 分钟内有人到场。</p>
              <span className="go">看这个项目 <Arrow s={13} /></span>
            </Link>
          </div>
        </div>
