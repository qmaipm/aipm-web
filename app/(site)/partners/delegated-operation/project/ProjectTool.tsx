...
import { CHOICES, EMPTY_ASSESSMENT, GRADES, POLICY, PARTNER_PATH, PROPERTY_TYPES, CONTACT_STAGES, ROLES, scoreProject, estimateReturns, validateAssessment, money, percent, type Assessment, type Role } from "@/lib/delegated-policy";
...
export default function ProjectTool({enabled,analyticsEnabled=false}:{enabled:boolean;analyticsEnabled?:boolean}) {
 ...
 function update(key:keyof Assessment,value:string){
  ...
  setAssessment(a=>({...a,[key]:value===""?null:Number(value)}));setError("");
 }
 const numeric=(key:keyof Assessment,label:string,hint:string,max:number,stepValue="any")=><div className="dp-field" key={key}><label htmlFor={`dp-${key}`}>{label}</label><input id={`dp-${key}`} type="number" inputMode="decimal" min={key==="area"?0.01:0} max={max} step={stepValue} value={assessment[key]??""} onChange={e=>update(key,e.target.value)} aria-describedby={`hint-${key}`} placeholder="暂不清楚可留空"/><small id={`hint-${key}`}>{hint}</small></div>;
 const choice=(key:keyof typeof CHOICES,label:string)=><div className="dp-field" key={key}><label htmlFor={`dp-${key}`}>{label}</label><select id={`dp-${key}`} value={assessment[key]??""} onChange={e=>update(key,e.target.value)}><option value="">暂不清楚，待补充</option>{CHOICES[key].map(([n,t])=><option key={n} value={n}>{t}（{n} 分）</option>)}</select></div>;
 ...
 const roleInput=<div className="dp-field"><label htmlFor="dp-role">预计参与角色</label><select id="dp-role" value={role} onChange={e=>setRole(e.target.value as Role)}>{Object.entries(ROLES).map(([k,r])=><option key={k} value={k}>{r.label} · 系数 {percent(r.factor)}</option>)}</select><small>{ROLES[role].work}。最终角色需书面确认。</small></div>;
 ...
 return <section className="dp-tool" id="project-tool"><div className="wrap">
  <noscript><p className="dp-message">预评级与在线报备需要启用 JavaScript。也可致电 020-8985 3580 联系受理人，或下载白皮书查看规则。</p></noscript>
  <ol className="dp-stepnav" aria-label="项目评估步骤">{["01 项目预评级","02 收益情景","03 提交报备"].map((t,i)=><li key={t}><button type="button" disabled={busy||!!receipt} aria-current={step===i?"step":undefined} onClick={()=>go(i)}>{t}</button></li>)}</ol>
  <div className="dp-tool-grid">
   <div className="dp-form-panel" ref={panel} tabIndex={-1}>
    ...
    {step===0&&<><h2 className="dp-step-title">填写你已经了解的项目情况</h2><p className="dp-small">所有评分项均可暂留空。信息仅在当前页面用于预评，点击最终提交前不会发送至受理台账；刷新页面会清除填写内容。</p>
     <fieldset><legend>01 区位与战略属性</legend>{choice("location","城市区位与产业集聚")}{choice("transport","交通可达性")}{choice("amenities","周边配套")}{choice("strategy","基础评分中的战略属性")}</fieldset>
     <fieldset><legend>02 面积与现有收入</legend>{numeric("area","实际管理面积（㎡）","以实际管理范围计算；符合统一委托与运营条件的项目可合并。",1e9)}{numeric("revenue","当前年度可确认物业收入（元／年）","填写物业项目收入，不是启盟酬金。不含未经确认的预测增收。",1e12)}</fieldset>
     <fieldset><legend>03 入住率与收入保障</legend>{numeric("occupancy","实际入住率（%）","填写真实比例，保障修正只作用于评分。",100)}{choice("guarantee","空置面积收入保障")}</fieldset>
     <fieldset><legend>04 现有人员编制</legend>{numeric("staff","实际在岗人数（人）","含自有人员及长期固定外包岗位，不含临时施工人员。",1e6,"1")}{result.density!==null&&<p className="dp-small">当前人员密度：每万㎡ {result.density.toFixed(2)} 人。人数及密度按 V2.0 分别计分。</p>}</fieldset>
     {scoreError&&<p className="dp-message" role="alert">{scoreError}</p>}
     <div className="dp-actions"><button className="btn btn-primary" type="button" onClick={()=>go(1)}>查看适用政策与测算</button><button className="btn btn-ghost" type="button" onClick={()=>go(2)}>先报备，后补资料</button></div>
    </>}
    {step===1&&<><h2 className="dp-step-title">按预计贡献，查看收益情景</h2>{roleInput}<label className="dp-checkbox"><input type="checkbox" checked={strategic} onChange={e=>setStrategic(e.target.checked)}/><span>拟申请 S 战略标签<small style={{display:"block"}}>需提供正式证明材料并经专项确认。勾选只用于比较情景，不代表标签已获批。</small></span></label>
     {!result.grade?<p className="dp-message">评分资料尚未完整，暂不能确认等级或测算金额。可返回补充，也可直接报备，由启盟共同评估。</p>:<>
      <h3>{GRADES[result.grade].label} · {ROLES[role].label}</h3>
     ...
     <div className="dp-actions"><button type="button" className="btn btn-primary" onClick={()=>go(2)}>带着项目继续报备</button><button type="button" className="btn btn-ghost" onClick={()=>go(0)}>返回修改项目资料</button></div>
    </>}
    {step===2&&<><h2 className="dp-step-title">{review?"核对资料，再确认提交":"留下真实机会和你的联系方式"}</h2>
     {!enabled&&<p className="dp-message" role="status">在线报备暂未开放，当前可以预评和准备资料。请联系 <a href="tel:02089853580">020-8985 3580</a> 或 <a href="mailto:liuziwen@aipm.cn">liuziwen@aipm.cn</a>；电话或邮件沟通后仍需受理人书面确认归属。</p>}
     {review?<div className="dp-review"><dl>{[["你的姓名",contact.name],["联系电话",contact.phone],["公司或机构",contact.company||"个人伙伴"],["项目",`${contact.city} · ${contact.project}`],["业主或主体",contact.owner],["业态",contact.propertyType],["决策人联系",contact.contactStage],["关系与推进",contact.relationship],["主要需求",contact.need],["预评级",result.grade?`${GRADES[result.grade].label} / ${result.total} 分，待核验`:"资料待补充"],["预计角色",ROLES[role].label],["S 标签",strategic?"拟申请，待专项确认":"未申请"]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><p className="dp-small">提交只生成受理回执。查重、身份与资料核验通过后，由启盟书面确认项目归属及保护期。</p><div className="dp-actions"><button type="button" className="btn btn-primary" disabled={!enabled||busy} onClick={submit}>{busy?"正在保存项目…":"确认并提交项目报备"}</button><button type="button" className="btn btn-ghost" disabled={busy} onClick={()=>setReview(false)}>返回修改</button></div></div>:<form onSubmit={preview}>
      ...
      {numeric("area","大致管理面积（㎡，可待补充）","与前面的预评级共享，无需重复填写。",1e9)}
      ...
      <div className="dp-actions"><button className="btn btn-primary" type="submit">核对报备资料</button><button className="btn btn-ghost" type="button" onClick={()=>go(0)}>返回预评级</button></div>
     </form>}
    </>}
    </>}
   </div>
   {!receipt&&<aside className="dp-result" aria-label="当前预评级结果"><span className="dp-eyebrow">当前预评级</span><strong className="dp-score">{result.total===null?"待补充":`${result.total} 分`}</strong><h2>{result.grade?`${GRADES[result.grade].label} · ${GRADES[result.grade].positioning}`:"填已知资料，不确定就留空"}</h2><dl className="dp-score-list">{result.dimensions.map(d=><div key={d.label}><dt>{d.label}</dt><dd>{d.score===null?"待补充":`${d.score} / ${d.max}`}</dd></div>)}</dl><p>未知项不按零分处理。评分完整后显示暂定等级，启盟需核验经营资料、现场和项目风险。</p>{strategic&&<p>S 标签：拟申请，待专项认定。</p>}<p><Link href={`${PARTNER_PATH}#policy`}>查看 V{POLICY.version} 合作政策</Link></p><p><a href="#scoring-rules">查看评分依据</a></p></aside>}
  </div>
 </div></section>;
}
